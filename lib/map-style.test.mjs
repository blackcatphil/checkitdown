...
test('RULE: the SKY joins the luminance chain — a horizon cannot outshine a building', () => {
  ...
  /* THE HAZE HAS LEFT THIS CHAIN — 2026-08-09. `--cid-map-fog` was asserted
     here because the `.cid-mapfog` scrim was a large-area SURFACE, on the same
     terms as the sky. That surface was removed by ruling, so the token no
     longer paints a field of anything: it survives only as `setSky`'s
     `fog-color`, which blends the ground into an off-screen horizon and moves
     0.01% of pixels at this camera.
     Asserting it here after the removal would be measuring a surface that is
     not drawn — the "passes while proving nothing" class this file has already
     been caught by twice. The SKY assertion above is untouched: a sky is still
...
/* REMOVED — 'RULE: labels stay legible THROUGH THE VALLEY HAZE', 2026-08-09.
   It measured label text against the halo composited under the `.cid-mapfog`
   scrim at alpha 0.9. That scrim no longer exists, so the composite it asserted
   is a state the product cannot render.
   NOTE WHY THIS IS A DELETION AND NOT A RESTATEMENT, since this file restates
   by default: the test would still have PASSED after the removal — it is pure
   token arithmetic and `--cid-map-fog` is still defined for `setSky`. It would
   have gone on reporting a green result about a surface nobody draws, which is
   worse than no test at all.
   THE RISK IT COVERED IS STILL BOUND, by the two assertions that remain and
   that between them are now the whole envelope:
     - 'label contrast holds at 4.5:1 against its halo'  — the land case
     - 'labels stay legible AGAINST THE HORIZON'         — the bright-band case,
       which is the worst backdrop a label can land on now that nothing is
       darkening the top of the frame.
   Removing the haze makes the far ground BRIGHTER, so the horizon case below is
   the one that got harder, and it is asserted. */
