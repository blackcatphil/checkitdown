...
test('RULE: the ARTERIALS stay purple — gold is allowed on the fine grid only', () => {
  ...
  for (const name of ['--cid-map-road-casing', '--cid-map-road-major']) {
    const c = C(name)
    assert.ok(c.b > c.r, `${name} is warm-dominant — gold is back on the arterials, which is how it became a surface`)
  }
  ...
  for (const name of ['--cid-map-land', '--cid-map-land-2', '--cid-map-building']) {
    assert.ok(C(name).b > C(name).r, `${name} is warm-dominant — gold has reached a FILL, which is a surface by definition`)
  }
})
