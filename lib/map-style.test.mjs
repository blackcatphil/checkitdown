...
test('RULE: the gold network is a hierarchy, and every step stays under a building', () => {
  /* An earlier rule said the tint belonged in the shadows and the bright end
     stayed neutral. Gold as the accent tier reverses that DELIBERATELY: the
     network is now the brightest thing on the ground. What survives is the
     constraint that actually protects the view — brightness belongs to the
     buildings — so it is asserted as an ordered chain rather than as a claim
     about which end is tinted. */
  ...
  assert.ok(ls.at(-1) < luminance(C('--cid-accent-500')),
    'the gold network outshines a lit building')
})
