...
test('RULE: gold outlines the arterials, it does not BE them', () => {
  /* Narrowed again, and the boundary has moved from "which roads" to "which
     ...
     at z10), so a gold casing under a purple body shows as an edge down each
     side — a line. The BODY is the surface, and it stays purple. */
...
test('RULE: gold never reaches a FILL, which is a surface by definition', () => {
  ...
  /* Hue distance, not channel dominance: the fills are three different families
     now (aubergine, moss, indigo) and only ONE of them is blue-dominant. A
...
test('RULE: the gold minor grid is held at texture weight', () => {
  ...
  assert.equal(find('highway_major_inner').paint['line-opacity'], undefined,
    'the arterials are purple and need no damping')
})
...
test('RULE: hover is gold, and is NOT the colour that means verified', () => {
  ...
  const value = C('--cid-value')
  assert.ok(hueGap(hover, value) > 60,
    `hover is ${hueGap(hover, value).toFixed(0)}° from --cid-value — hover must not spend the verified colour`)
...
test('RULE: the map tokens exist in BOTH themes', () => {
  ...
  for (const name of [...GROUND, '--cid-map-boundary', '--cid-map-building-lit', '--cid-map-hover', ...GOLD_TEXT, ...GOLD_FILL, '--cid-gold-line', '--cid-gold-wash']) {
