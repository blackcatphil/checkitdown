...
import {
  cellFor, composite, diagnoseOverdue, firstReadingOn, isoWeekStart, isFault,
  readingDateLabel, render,
...
/* ─── WHICH FAULT IS IT? ─────────────────────────────────────────────────
   ⚠️ ON 17 AUGUST THE ENGINE WILL READ OVERDUE, and the cause will be that
   nothing refreshes the matview — 021 describes an hourly refresh and ships no
   scheduler. A fault that blames the producer for a missing cron sends somebody
   to debug analytics for a day. These two must read differently. */

test('OVERDUE names the ROLL-UP when events are arriving and nothing refreshes', () => {
  const msg = diagnoseOverdue({
    refreshedAt: new Date(Date.UTC(2026, 7, 12, 6)),   // six hours ago
    latestEvent: new Date(Date.UTC(2026, 7, 12, 11)),  // events still coming
    now: new Date(Date.UTC(2026, 7, 12, 12)),
  })
  assert.match(msg, /roll-up has not refreshed/)
  assert.match(msg, /NOT a producer fault/)
  assert.doesNotMatch(msg, /producer has stopped/)
})

test('OVERDUE names the PRODUCER when the roll-up is current and empty', () => {
  const msg = diagnoseOverdue({
    refreshedAt: new Date(Date.UTC(2026, 7, 12, 11, 30)), // half an hour ago
    latestEvent: new Date(Date.UTC(2026, 7, 5)),          // a week of silence
    now: new Date(Date.UTC(2026, 7, 12, 12)),
  })
  assert.match(msg, /producer has stopped/)
  assert.doesNotMatch(msg, /scheduler/)
})

test('never refreshed at all is the strongest form of stale, and says so', () => {
  const msg = diagnoseOverdue({
    refreshedAt: null, latestEvent: new Date(Date.UTC(2026, 7, 12, 11)),
    now: new Date(Date.UTC(2026, 7, 12, 12)),
  })
  assert.match(msg, /NEVER been refreshed/)
  assert.match(msg, /not a producer fault/i)
})
