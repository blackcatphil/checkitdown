...
/* ═══════════════════════════════════════════════════════════════════════════
   A GUARD THAT PINS A STRING NOBODY PRINTS FAILS FOREVER.

   Every probe here reports `0 failed` on its own, so ci.yml adds a second
   layer: `grep -q 'PASS  <assertion name>'` over the log, which catches an
   assertion that stopped RUNNING rather than one that started failing — a
   probe that silently skipped a whole section still exits 0.

   That layer has a failure mode of its own, and it cost this repo four days.
   The installed-app assertion in pwa-probe.mjs was deliberately SPLIT in two
   when the sign-in form gained a password path, and the pin naming the old
   single assertion was not moved with it. From that commit on, the probe
   passed, the guard could never match, and main was red on a string nothing
   printed. Nothing in the suite could see it, because the mistake was not in
   the probe and not in the workflow — it was in the relationship between them,
   which is the same gap the top of this file exists to close.

   So: every `PASS` pin in the workflows must be text some script can actually
   emit. It runs offline in `test:unit`, so it fails at commit time rather than
   at 01:59 in CI.
   ═══════════════════════════════════════════════════════════════════════════ */

const SCRIPTS = Object.fromEntries(
  globSync('scripts/**/*.mjs').map((f) => [f, readFileSync(f, 'utf8')]),
)

/**
 * Every quoted assertion name a workflow pins.
 *
 * ⚠️ MATCHED ON THE `PASS  `/`RED  ` PREFIX, NOT ON `grep -q`, because the pins
 * do not all sit next to a grep. The six RED pins in the events gate are values
 * in a `for red in …` list that a `grep -qF "$red"` reads one at a time — a
 * scanner keyed to the grep would find the variable and miss all six, which is
 * the same blind spot in a different spelling.
 */
function grepPins(text, file) {
  const out = []
  for (const m of text.matchAll(/(['"])((?:PASS|RED)  [^'"]*)\1/g)) {
    out.push({ file, line: text.slice(0, m.index).split('\n').length, text: m[2] })
  }
  return out
}

const PASS_PINS = Object.entries(WORKFLOWS).flatMap(([f, t]) => grepPins(t, f))

/**
 * ⚠️ PINS THE PROBE BUILDS AT RUNTIME, so no literal match can exist.
 *
 * Each entry must still name a `literal` fragment that IS in the sources, and
 * that fragment is asserted below. An allowlist whose entries pinned nothing
 * would be a way to wave a dead guard through, which is the exact bug this
 * whole block exists to catch.
 */
const INTERPOLATED = [
  { pin: 'facts: no horizontal overflow',
    literal: ': no horizontal overflow',
    why: '`${label}: …` — label comes from the page list in mobile-probe' },
  { pin: 'facts: the HEADER nav is gone',
    literal: ': the HEADER nav is gone',
    why: 'same `${label}: …` template, same list' },
  { pin: 'install (Android, prompt offered): and the detector CAN see a fill',
    literal: ': and the detector CAN see a fill',
    why: '`${name}: …` — name comes from the palette-probe page table' },
  { pin: 'the skyline draws at iPhone SE',
    literal: 'the skyline draws at ',
    why: 'a PREFIX of `the skyline draws at ${label} (…)`, label "iPhone SE, Safari chrome"' },
]

test('the pin parser actually finds pins — without this every assertion below is vacuous', () => {
  assert.ok(PASS_PINS.length >= 35,
    `only ${PASS_PINS.length} PASS/RED pins were parsed out of the workflows — the `
    + 'regex stopped matching, and a scan that finds nothing agrees with everything')
  assert.ok(Object.keys(SCRIPTS).length >= 20,
    `only ${Object.keys(SCRIPTS).length} probe sources were read`)
})

test('every PASS string pinned in a workflow is one some script can actually print', () => {
  const printable = Object.values(SCRIPTS)
  const dead = []
  for (const p of PASS_PINS) {
    const needle = p.text.replace(/^(PASS|RED)  /, '')
    if (printable.some((src) => src.includes(needle))) continue
    if (INTERPOLATED.some((i) => i.pin === needle)) continue
    dead.push(`${p.file}:${p.line} pins "${p.text}" — no script prints it`)
  }
  assert.deepEqual(dead, [],
    'a guard is pinning an assertion name nothing emits, so it can never match '
    + 'and the job fails forever:\n  ' + dead.join('\n  '))
})

test('the interpolated allowlist pins something real, and nothing retired', () => {
  const printable = Object.values(SCRIPTS)
  for (const i of INTERPOLATED) {
    assert.ok(printable.some((src) => src.includes(i.literal)),
      `the allowlist excuses "${i.pin}" on the grounds that ${i.why}, but its `
      + `literal fragment "${i.literal}" is in no script either — the template `
      + 'it claims to come from is gone, so the pin is dead after all')
    assert.ok(PASS_PINS.some((p) => p.text.replace(/^(PASS|RED)  /, '') === i.pin),
      `the allowlist still excuses "${i.pin}", which no workflow pins any more `
      + '— delete the entry rather than leaving it to excuse a future typo')
  }
})
