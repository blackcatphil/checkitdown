...
test('it draws SOMETHING for every extruded mass in the real data', () => {
  ...
  const feats = ROOM_FOOTPRINTS.features
    ...
    .map((f, i) => ({ ring: f.geometry.coordinates[0], height: f.properties.height, stagger: i / 16 * WIRE_STAGGER }))
  const g = buildWireGeometry(feats, zPerMeterAt)
  assert.equal(feats.length, 16, 'the 16 cited masses')
  assert.ok(g.segments > 200, `only ${g.segments} segments for 16 buildings`)
